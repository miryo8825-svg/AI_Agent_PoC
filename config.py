...
1. 【クエリ分析】
   ...
   1.4. 現在日時 = {today} を基準に期間を特定し、`marklines_search`のフィルタ条件を構築する（期間指定時は必ず含めること）。
...
- **【Web検索結果の分離】google_searchの検索結果を使用する場合、レポート下部に以下の欄を設けて別レポートとして記載すること(見出し文言は固定とする)**
      ...
      *google_searchの検索結果を使用しない場合、`外部Web検索`の欄は不要。*
